...
% DISCOVER_ENGINES - Find all execution engine plugins
    ...
    engines = containers.Map('KeyType', 'char', 'ValueType', 'any');
    
    if ~exist(base_path, 'dir')
        warning('Maestro:DirectoryNotFound', ...
            'Engines directory not found: %s', base_path);
        return;
    end
    
    % Find all schema.json files in engine directories
    ...
    for i = 1:length(all_schemas)
        ...
        try
            ...
            % Validate it's an engine schema (handle $schema -> x_schema conversion)
            ...
            if ~strcmp(schema_field, 'maestro-engine-v1')
                continue;
            end
            
            % Validate required fields
            required_fields = {'engine_type', 'version', 'implementation'};
            missing = {};
            ...
            if ~isempty(missing)
                warning('Maestro:InvalidSchema', ...
                    'Engine schema missing required fields: %s\nFile: %s', ...
            ...
            engine_path = fileparts(schema_file);
            
            % Create function handle
            func_name = schema.implementation.function;
            try
                execute_func = str2func(func_name);
            catch ME
            ...
            % Store engine info
            engine_info = struct();
            engine_info.schema = schema;
            engine_info.execute_func = execute_func;
            engine_info.path = engine_path;
            engine_info.schema_file = schema_file;
            
            engines(engine_type) = engine_info;
            
        catch ME
            warning('Maestro:DiscoveryError', ...
                'Error loading engine from %s: %s', ...
