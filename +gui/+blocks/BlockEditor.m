    ...
    methods
       ...
       function createParameterField(app, parentLayout, param_name, param_spec, row)
        ...
        function onArrayRemove(app, listbox)
            selectedValue = listbox.Value; % Returns cell array of selected strings
            if isempty(selectedValue)
                uialert(app.UIFigure, 'Select an item to remove.', 'Remove Item');
                return;
            end
             % For single selection listbox, Value is a cell with one string
             % Find the index of this string in Items
            selectedIndex = find(strcmp(listbox.Items, selectedValue{1}), 1);
            if ~isempty(selectedIndex)
                listbox.Items(selectedIndex) = [];
                listbox.Value = {}; % Clear selection
            else
                 gui.common.error_dialog.show_general_error(app.UIFigure, MException('App:Error','Selection mismatch error.'), 'Error');
                 listbox.Value = {};
            end
        end
