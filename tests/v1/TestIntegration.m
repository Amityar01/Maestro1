classdef TestIntegration < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testForeperiodProbabilisticOutcomes(testCase)
            % Test foreperiod with multiple probabilistic outcomes

            % Step 1: Configure foreperiod with probabilistic outcomes
            config = struct();
            config.cue = struct('stimulus_ref', 'tone_1000hz', 'duration_ms', 50);

            % Define multiple outcomes with probabilities
            config.outcomes = {
                struct('stimulus_ref', 'tone_1200hz', 'duration_ms', 50, ...
                       'probability', 0.2, 'label', 'high')
                struct('stimulus_ref', 'tone_800hz', 'duration_ms', 50, ...
                       'probability', 0.7, 'label', 'low')
                struct('stimulus_ref', 'omission', 'duration_ms', 0, ...
                       'probability', 0.1, 'label', 'omission', 'is_omission', true)
            };

            config.foreperiods = [500];  % Fixed foreperiod
            config.foreperiod_probs = [1.0];
            config.selection = struct('mode', 'balanced_shuffle', 'seed', 888);
            config.iti = struct('value', 1000);

            % Step 2: Generate trial plan
            adapter = v1.paradigms.ForeperiodAdapter();
            trial_plan = adapter.generate_trial_plan(config, 30);

            testCase.verifyEqual(trial_plan.n_trials, 30);

            % Count outcome types
            outcome_counts = struct('high', 0, 'low', 0, 'omission', 0);
            for i = 1:length(trial_plan.trials)
                trial = trial_plan.trials{i};
                outcome = trial.elements{2};

                if isfield(outcome, 'label')
                    outcome_counts.(outcome.label) = outcome_counts.(outcome.label) + 1;
                end
            end

            % Verify probabilities (balanced shuffle should give exact counts)
            testCase.verifyEqual(outcome_counts.high, 6, 'Expected 6 high outcomes (0.2 * 30)');
            testCase.verifyEqual(outcome_counts.low, 21, 'Expected 21 low outcomes (0.7 * 30)');
            testCase.verifyEqual(outcome_counts.omission, 3, 'Expected 3 omissions (0.1 * 30)');

            % Step 3: Build element table
            builder = v1.compilation.PatternBuilderCore();
            element_table = builder.build(trial_plan);

            testCase.verifyEqual(height(element_table), 60, 'Should have 60 elements (30 trials × 2)');

            % Verify omission elements
            omission_elements = element_table(strcmp(element_table.stimulus_ref, 'omission'), :);
            testCase.verifyEqual(height(omission_elements), 3, 'Should have 3 omission elements');

            % Step 4: Compile (skip omission elements in compilation)
            compiler = v1.compilation.CompilerCore();
            element_table.ttl_code = ones(height(element_table), 1);

            % Filter out omission elements for compilation
            non_omission_table = element_table(~strcmp(element_table.stimulus_ref, 'omission'), :);

            seq_file = compiler.compile(non_omission_table, testCase.stimulus_library, ...
                                       testCase.fs_hz, testCase.context);

            % Should have 57 events (30 cues + 27 outcomes, 3 omitted)
            testCase.verifyEqual(height(seq_file.events), 57);
            ...
            testCase.verifyTrue(result.success);
            testCase.verifyEqual(result.events_played, 57);
        end
